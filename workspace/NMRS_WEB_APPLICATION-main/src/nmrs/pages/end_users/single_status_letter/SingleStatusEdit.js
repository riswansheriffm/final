import { useEffect, useRef, useState } from "react";
import { useLocation, useNavigate } from "react-router-dom";
import { Button, Col, Container, Form, Row } from "reactstrap";
import Swal from "sweetalert2";
import "sweetalert2/dist/sweetalert2.css";
import { useDataWithId } from "../../../hooks/useDataWithId";
import useDate from "../../../hooks/useDate";
import TextInput from "../../../components/forms/TextInput";
import TelInput from "../../../components/forms/TelInput";
import DateInput from "../../../components/forms/DateInput";
import FormSelect from "../../../components/forms/FormSelect";
import FormFileInput from "../../../components/forms/FormFileInput";
import NumberInput from "../../../components/forms/NumberInput";
import { toast } from "react-toastify";

export default function SingleStatusEdit({ id, setModal, editWithKey }) {
  const formatDate = useDate();

  const location = useLocation();

  const routeName = location.pathname.split("/")[1];
  const dbName = routeName;

  const singlestatuslist = useDataWithId({ dbName, id });

  const [details, setDetails] = useState({
    aName: "",
    aNumber: "",
    aDob: "",
    aFName: "",
    aMName: "",
    aAddress: "",
    county: "",
    district: "",
    zipCode: "",
    aLBC: "",
    aId: "",
    aSDP: "",
    aSDA: "",
    aLC1: "",
    doA: formatDate,
    status: "Applied",
  });

  const [errorMsg, setErrorMsg] = useState({
    aName: "",
    aNumber: "",
    aDob: "",
    aFName: "",
    aMName: "",
    aAddress: "",
    county: "",
    district: "",
    zipCode: "",
    aLBC: "",
    aId: "",
    aSDP: "",
    aSDA: "",
    aLC1: "",
  });
  const dateObj = new Date();
  const year = dateObj.getFullYear() - 18;
  const month = dateObj.getMonth() + 1;
  const date = dateObj.getDate();

  const showSuccessAlert = () => {
    Swal.fire({
      icon: "success",
      title: "Success!",
      text: "Details Updated Successfully",

      confirmButtonText: "OK",
    }).then((result) => {
      if (result.isConfirmed) {
        // navigate(`/${routeName}`);
        setModal(false);
      }
    });
  };
  const mobileNumRegex = /^\+256[1-9]\d{8}$/;

  const [files, setFiles] = useState({});

  // useEffect(() => {
  //     console.log(details, "singlestatuslist");
  // }, [details]);

  useEffect(() => {
    if (singlestatuslist) {
      setDetails(singlestatuslist);
    }
    const detailsKeys = Object.keys(singlestatuslist || {});
    detailsKeys.forEach((value, index) => {
      if (index >= 8) {
        if (singlestatuslist[value]) {
          const reader = new FileReader();

          if (singlestatuslist[value] instanceof Blob) {
            reader.readAsDataURL(singlestatuslist[value]);
          }
          reader.onload = function (e) {
            setFiles((elem) => {
              return { ...elem, [value]: e.target.result };
            });
          };
        }
      }
    });
  }, [singlestatuslist]);

  const [step, setStep] = useState(1);
  const showSaved = () => {
    Swal.fire({
      icon: "success",
      title: "Saved!",
      text: "Details Saved Successfully",
      confirmButtonText: "OK",
    }).then((result) => {
      if (result.isConfirmed) {
        setModal(false);
      }
    });
  };

  const handleSave = (e) => {
    e.preventDefault();
    if (step === 1) {
      if (
        !details.aName &&
        !details.aNumber &&
        !details.aDob &&
        !details.aFName &&
        !details.aMName &&
        !details.aAddress &&
        !details.county &&
        !details.district &&
        !details.zipCode
      ) {
        toast.error(`Please Complete All The Information`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aName) {
        toast.error(`Please Enter Applicant Name`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aNumber) {
        toast.error(`Please Enter Applicant Number`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!mobileNumRegex.test(details.aNumber)) {
        filled.current.one = false;
        toast.error(`Please Enter a Valid Mobile Number`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aDob) {
        toast.error(`Please Enter Date Of Birth`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aFName) {
        toast.error(`Please Enter Father Name`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aMName) {
        toast.error(`Please Enter Mother Name`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aAddress) {
        toast.error(`Please Enter Residential Address`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.county) {
        toast.error(`Please Select County`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.district) {
        toast.error(`Please Select District`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.zipCode) {
        toast.error(`Please Enter Zip Code`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else {
        showSaved();
      }
    }
    if (step === 2) {
      if (
        !details.aLBC &&
        !details.aId &&
        !details.aSDP &&
        !details.aSDA &&
        !details.aLC1
      ) {
        toast.error(`Please Complete All The Information`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aLBC) {
        toast.error(`Please Attach Long Birth Certificate`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aId) {
        toast.error(`Please Attach National Id / Passport`, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aSDA) {
        toast.error(`Please Attach Statutory Declaration Of Applicant `, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aSDP) {
        toast.error(`Please Attach Statutory Declaration Of Parent `, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else if (!details.aLC1) {
        toast.error(`Please Attach ChairPerson Recommendation Letter `, {
          theme: "colored",
          toastId: "error-toast",
        });
      } else {
        showSaved();
      }
    }
  };
  const handleReset = (e) => {
    e.preventDefault();
    setDetails({
      aName: "",
      aNumber: "",
      aDob: "",
      aFName: "",
      aMName: "",
      aAddress: "",
      county: "",
      district: "",
      zipCode: "",
      aLBC: "",
      aId: "",
      aSDP: "",
      aSDA: "",
      aLC1: "",
    });
  };
  const handlePrevious = (e) => {
    e.preventDefault();

    setStep(step - 1);
  };
  const filled = useRef({});

  const handleChange = (e) => {
    e.preventDefault();

    const detailsKeys = Object.keys(details);

    detailsKeys.forEach((value) => {
      if (e.target.type !== "file") {
        if (e.target.name !== "aDob") {
          if (e.target.name === value) {
            setDetails((elem) => {
              return { ...elem, [value]: e.target.value };
            });
            setErrorMsg((elem) => {
              return { ...elem, [value]: "" };
            });
          }
        } else {
          if (e.target.name === value) {
            setDetails((elem) => {
              return { ...elem, [value]: e.target.value };
            });

            if (year === Number(e.target.value.split("-")[0])) {
              if (month === Number(e.target.value.split("-")[1])) {
                if (date >= Number(e.target.value.split("-")[2])) {
                  setErrorMsg((elem) => {
                    return { ...elem, [value]: "" };
                  });
                } else {
                  setErrorMsg((elem) => {
                    return { ...elem, [value]: "Applicant Must Be Above 18" };
                  });
                }
              } else if (month > Number(e.target.value.split("-")[1])) {
                setErrorMsg((elem) => {
                  return { ...elem, [value]: "" };
                });
              } else {
                setErrorMsg((elem) => {
                  return { ...elem, [value]: "Applicant Must Be Above 18" };
                });
              }
            } else if (year > Number(e.target.value.split("-")[0])) {
              setErrorMsg((elem) => {
                return { ...elem, [value]: "" };
              });
            } else {
              setErrorMsg((elem) => {
                return { ...elem, [value]: "Applicant Must Be Above 18" };
              });
            }
          }
        }
      } else {
        if (e.target.name === value) {
          if (!e.target.value && !files[value]) {
            return null;
          }
          const reader = new FileReader();
          reader.onload = function (e) {
            setFiles((elem) => {
              return { ...elem, [value]: e.target.result };
            });
          };

          reader.readAsDataURL(e.target.files[0] || files[value]);
          setDetails((elem) => {
            return { ...elem, [value]: e.target.files[0] };
          });
          setErrorMsg((elem) => {
            return { ...elem, [value]: "" };
          });
        }
      }
    });
  };

  const handleNext = (e) => {
    e.preventDefault();
    const errorMsgKeys = Object.keys(errorMsg);

    if (
      !details.aName &&
      !details.aNumber &&
      !details.aDob &&
      !details.aFName &&
      !details.aMName &&
      !details.aAddress &&
      !details.county &&
      !details.district &&
      !details.zipCode
    ) {
      toast.error(`Please Complete All The Information`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aName) {
      toast.error(`Please Enter Applicant Name`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aNumber) {
      toast.error(`Please Enter Applicant Number`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!mobileNumRegex.test(details.aNumber)) {
      toast.error(`Please Enter a Valid Mobile Number`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aDob) {
      toast.error(`Please Enter Date Of Birth`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aFName) {
      toast.error(`Please Enter Father Name`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aMName) {
      toast.error(`Please Enter Mother Name`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aAddress) {
      toast.error(`Please Enter Residential Address`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.county) {
      toast.error(`Please Select County`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.district) {
      toast.error(`Please Select District`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.zipCode) {
      toast.error(`Please Enter Zip Code`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else {
      setStep(step + 1);
      filled.current.one = true;
    }
    if (
      details.aName &&
      details.aNumber &&
      details.aDob &&
      details.aFName &&
      details.aMName &&
      details.aAddress &&
      details.county &&
      details.district &&
      details.zipCode
    ) {
      if (year === Number(details.aDob.split("-")[0])) {
        if (month === Number(details.aDob.split("-")[1])) {
          if (date >= Number(details.aDob.split("-")[2])) {
          } else filled.current.one = false;
        } else if (month > Number(details.aDob.split("-")[1])) {
        } else filled.current.one = false;
      } else if (year > Number(details.aDob.split("-")[0])) {
      } else filled.current.one = false;
    } else filled.current.one = false;
  };
  const handleSubmit = (e) => {
    e.preventDefault();
    if (
      !details.aLBC &&
      !details.aId &&
      !details.aSDP &&
      !details.aSDA &&
      !details.aLC1
    ) {
      toast.error(`Please Complete All The Information`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aLBC) {
      toast.error(`Please Attach Long Birth Certificate`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aId) {
      toast.error(`Please Attach National Id / Passport`, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aSDA) {
      toast.error(`Please Attach Statutory Declaration Of Applicant `, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aSDP) {
      toast.error(`Please Attach Statutory Declaration Of Parent `, {
        theme: "colored",
        toastId: "error-toast",
      });
    } else if (!details.aLC1) {
      toast.error(`Please Attach ChairPerson Recommendation Letter `, {
        theme: "colored",
        toastId: "error-toast",
      });
    }

    if (
      details.aLBC &&
      details.aId &&
      details.aSDP &&
      details.aSDA &&
      details.aLC1
    ) {
      editWithKey(details, id);
      filled.current.two = true;

      showSuccessAlert();
    } else filled.current.two = false;
  };
  return (
    <>
      <Container fluid>
        <Row className="justify-content-center align-items-center mb-3 ">
          <Col role="button" onClick={() => setStep(1)}>
            <h2
              className={
                step === 1
                  ? "border-bottom-1 text-center pb-2"
                  : "text-center pb-2"
              }
            >
              Personal Information
              {filled.current.one === true && (
                <i class="ml-1 text-success fa-solid fa-circle-check"></i>
              )}
              {filled.current.one === false && (
                <i class="ml-1 fa-solid text-danger fa-circle-xmark"></i>
              )}
            </h2>
          </Col>

          <Col
            role="button"
            onClick={step === 1 ? handleNext : () => setStep(2)}
          >
            <h2
              className={
                step === 2
                  ? "border-bottom-1 text-center pb-2"
                  : "text-center pb-2"
              }
            >
              Documents Needed
              {filled.current.two === true && (
                <i class="ml-1 text-success fa-solid fa-circle-check"></i>
              )}
              {filled.current.two === false && (
                <i class="ml-1 fa-solid text-danger fa-circle-xmark"></i>
              )}
            </h2>
          </Col>
        </Row>
        <Form onSubmit={handleSubmit}>
          <div className="border-3">
            <Row>
              {step === 1 && (
                <>
                  <TextInput
                    id="aName"
                    labelText="Applicant Name"
                    onChange={handleChange}
                    hasError={errorMsg.aName}
                    value={details.aName}
                  />
                  <TelInput
                    id="aNumber"
                    labelText="Applicant's Mobile Number"
                    onChange={handleChange}
                    value={details.aNumber}
                    hasError={errorMsg.aNumber}
                  />
                  <DateInput
                    id="aDob"
                    labelText="Applicant Date Of Birth"
                    onChange={handleChange}
                    hasError={errorMsg.aDob}
                    value={details.aDob}
                    max={`${year}-${month < 10 ? "0" + month : month}-${
                      date < 10 ? "0" + date : date
                    }`}
                  />

                  <TextInput
                    id="aFName"
                    labelText="Applicant's Father's Name"
                    onChange={handleChange}
                    hasError={errorMsg.aFName}
                    value={details.aFName}
                  />
                  <TextInput
                    id="aMName"
                    labelText="Applicant's Mother's Name"
                    onChange={handleChange}
                    hasError={errorMsg.aMName}
                    value={details.aMName}
                  />
                  <TextInput
                    id="aAddress"
                    labelText="Applicant's Address"
                    onChange={handleChange}
                    hasError={errorMsg.aAddress}
                    value={details.aAddress}
                  />
                  <FormSelect
                    id="county"
                    labelText="County"
                    onChange={handleChange}
                    hasError={errorMsg.county}
                    value={details.county}
                  >
                    <option value="">Select County</option>
                    <option>county</option>
                  </FormSelect>
                  <FormSelect
                    id="district"
                    labelText="District"
                    onChange={handleChange}
                    hasError={errorMsg.district}
                    value={details.district}
                  >
                    <option value="">Select District</option>
                    <option>District</option>
                  </FormSelect>
                  <NumberInput
                    id="zipCode"
                    labelText="Zip Code"
                    onChange={handleChange}
                    hasError={errorMsg.zipCode}
                    value={details.zipCode}
                  />
                </>
              )}
              {step === 2 && (
                <>
                  <FormFileInput
                    labelText="Applicant's National Id"
                    id="aLBC"
                    onChange={handleChange}
                    hasError={errorMsg.aLBC}
                    value={details.aLBC}
                    src={files.aLBC}
                  />
                  <FormFileInput
                    labelText="Long Birth Certificate"
                    id="aId"
                    onChange={handleChange}
                    hasError={errorMsg.aId}
                    value={details.aId}
                    src={files.aId}
                  />
                  <FormFileInput
                    labelText="Statutory Declaration Of Applicant"
                    id="aSDA"
                    onChange={handleChange}
                    hasError={errorMsg.aSDA}
                    value={details?.aSDA}
                    src={files?.aSDA}
                  />
                  <FormFileInput
                    labelText="Statutory Declaration Of Parent"
                    id="aSDP"
                    onChange={handleChange}
                    hasError={errorMsg.aSDP}
                    value={details?.aSDP}
                    src={files?.aSDP}
                  />

                  <FormFileInput
                    labelText="LC1 ChairPerson Recommendation Letter"
                    id="aLC1"
                    onChange={handleChange}
                    hasError={errorMsg.aLC1}
                    value={details?.aLC1}
                    src={files?.aLC1}
                  />
                </>
              )}
            </Row>
          </div>
          <Row className="justify-content-center">
            <Button
              onClick={handleSave}
              className="saveBtn text-white"
              size="md"
            >
              <i className="fa-regular fa-floppy-disk text-white mr-1"></i>Save
            </Button>
            {step !== 1 && (
              <Button
                className="bg-primary text-white"
                size="md"
                onClick={handlePrevious}
              >
                <i className="fa-solid fa-arrow-left text-white mr-1"></i>
                Previous
              </Button>
            )}
            {step === 2 ? (
              <Button className="bg-usrb text-white" size="md" type="submit">
                <i className="fa-regular fa-paper-plane mr-1"></i>
                Submit
              </Button>
            ) : (
              <Button
                className="bg-usrb text-white"
                size="md"
                onClick={handleNext}
              >
                Next
                <i className="fa-solid fa-arrow-right text-white ml-1"></i>
              </Button>
            )}

            <Button
              className="bg-danger text-white"
              onClick={handleReset}
              size="md"
            >
              <i className="fa-solid fa-clock-rotate-left text-white mr-1"></i>
              Reset
            </Button>
          </Row>
        </Form>
      </Container>
    </>
  );
}
